...
U praksi se koriste alati za proveravanje modela koji implementiraju neki od algoritama za proveravanje modela. Spin je jedan od  alata za verifikaciju softvera koji podržava zadavanje zahteva LTL formulama\cite{holzmann1997model}. Ovi programi obično rade tako što zahteve zadate LTL formulama pretvaraju u konačne automate. Kao što se vidi u algoritmu \ref{alg:katl} uzima se negacija LTL formule i pravi se automat nad njom, zatim se napravi proizvod dobijenog automata i automata koji predstavlja sistem i vrši se provera ispunjenosti uslova \cite{gastin2001fast}.

Jedan primer kako se LTL formulom može zadati zahtev je sledeći. Ako posmatramo problem dodele nedeljivih resursa procesima, i formula $\alpha$ predstavlja zahtev za resursom a formula $\beta$ predstavlja realizaciju tog zahteva, onda formula $G(\alpha \rightarrow F \beta)$ označava da će svaki zahtev za resursom biti opslužen\cite{formverif}.
...
Kao što je već ranije pomenuto izražajnost LTL i CTL nije jednaka\ref{fig:ltlctlex}. Iz tog razloga i CTL su našle svoju primenu u proveravanju modela \cite{rozier2011linear}
Dakle model sistema je dat kao struktura, konačan automat, a specifikacija zahteva sistema je zadata formulama CTL-a. 
Glavna tehnika koja se ovde koristi za proveru modela je obilazak grafa pretagom u širinu \cite{abate2007one}.
